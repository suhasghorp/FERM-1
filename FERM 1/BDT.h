...
namespace week6 {
	...
	struct bdt_functor : Functor<double>
	{
		double m_b;
		int m_num_periods;
		vector<double> m_spot_rates;

		bdt_functor(double b, int num_periods, vector<double> spot_rates) : Functor<double>(num_periods, num_periods), m_b(b), m_num_periods(num_periods), m_spot_rates(spot_rates) {}
		int operator()(const Eigen::VectorXd &x, Eigen::VectorXd &fvec) const
		{
			vector<double> vec(x.data(), x.data() + x.rows() * x.cols());
			vector<vector<double>> short_rates = week6::bdt_short_rate_lattice(m_spot_rates[0], vec, m_b, m_num_periods);
			vector<vector<double>> bdt_elementary_prices = week6::bdt_elementary_lattice(short_rates, 0.5);
	...
	vector<vector<double>> buildBDT(vector<double>& yield_curve, vector<double>& a_guesses, double b, int num_periods) {

		Eigen::VectorXd x = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(a_guesses.data(), a_guesses.size());		

		bdt_functor functor(b, num_periods, yield_curve);
		Eigen::NumericalDiff<bdt_functor> numDiff(functor);
		Eigen::LevenbergMarquardt<Eigen::NumericalDiff<bdt_functor>, double> lm(numDiff);
